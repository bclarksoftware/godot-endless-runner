...
var cam_rotation_speed: float = 5.0
...
func _process(delta):
	#var t = int(Time.get_ticks_msec() * 1000)

	# camera shake
	#if shake_intensity > 0.0:
	#	shake_intensity -= delta
	#	$ANCHOR/MESH/Camera3D.transform.origin = cam_position + 2.0 * shake_intensity * Vector3(cos(0.05 * t), sin(0.05 * t), 0.0)
	...
	$ANCHOR/MESH.transform.origin.x = lerp($ANCHOR/MESH.transform.origin.x, (lane_index * 5.0) - 5.0, delta * 15.0)
	...
	#if shake_intensity <= 0.0:
	#	$ANCHOR/MESH/Camera3D.transform.origin.x = 0.35 * cos(0.0075 * t)
	#	$ANCHOR/MESH/Camera3D.transform.origin.y = cam_position.y + 0.35 * sin(0.0075 * t)
